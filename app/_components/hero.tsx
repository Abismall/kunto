...
export default function Hero() {
  ...
  const isFirstLoad = sessionStorage.getItem("animate-hero");
    ...
    >
      <div className="container flex px-5 py-24 pt-56 md:flex-row flex-col items-center justify-center md:justify-start">
        <div className="lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-14 md:mb-0 items-center text-center">
          <h1
            className={`sm:text-2xl text-3xl lg:text-8xl mb-4 text-secondary ${
              isFirstLoad ? "animate-slideInRight" : ""
            }`}
          >
            Anna kuntosi kohota
          </h1>
          <p
            className={`mb-10 leading-relaxed lg:text-2xl opacity-0 ${
              isFirstLoad ? "animate-slideDown" : ""
            }`}
            style={{
          ...
          <div className="flex justify-center md:justify-start gap-6">
            <Link
              ...
              className={`inline-flex text-#333333 border border-secondary py-2 px-6 md:py-3 md:px-8 focus:outline-none rounded text-lg md:text-3xl opacity-0 ${
