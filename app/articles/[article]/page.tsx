...
const fetchContent = (file: string) => {
  try {
    const filePath = path.join(process.cwd(), 'public', 'articles', file);
...
export default async function Page({
  ...
  return (
  <div className="flex flex-col items-center min-h-screen min-w-screen mt-12 p-6">
      <ReactMarkdown className="prose-sm lg:prose-2xl md:prose-2xl xl:prose-2xl" rehypePlugins={[rehypeRaw]}>
        {content}
      </ReactMarkdown>
    </div>
  );
